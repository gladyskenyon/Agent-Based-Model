...
class Agent:
    ...
    def __init__ (self, environment, agents, neighbourhood): # Passes in varibale lists to link to objects
        ...
        self._x = random.randint(0,300) # Set starting x co-ordinates randomly in the field between 0 and 300
        self._y = random.randint(0,300) # Set starting y co-ordinates randomly between 0 and 300
    ...
    # Create method for sheep to move    
    def move(self): # Take in self argument
        
        if random.random() < 0.5: # If random number is less than 0.5
            self._x = (self._x + 1) % 300 # Add 1 to x co-ordinate of sheep 
        else:                             # If random number is more than 0.5
            self._x = (self._x - 1) % 300 # Minus 1 from x co-ordinate of sheep
        
        if random.random() < 0.5:         # # If random number is less than 0.5
            self._y = (self._y + 1) % 300 # Add 1 to y co-ordinate of sheep
        else:                             # # If random number is more than 0.5
            self._y = (self._y - 1) % 300 # Minus 1 from y co-ordinate of sheep
    ...
    def eat(self): # Take in self argument
        ...
        # Use elif to mean True    
        elif self.environment[self._y][self._x] >0: # If the field has more than 0        
            self.environment[self._y][self._x] -= 1 # Minus 1 from the field       
            self.store += 1 # Add 1 to sheep stomach
    ...
    def sick(self):
        ...
        if self.store >= 100: # If sheeps store more than 100 grass
    ...
    # Create method for sheep to share info with others    
